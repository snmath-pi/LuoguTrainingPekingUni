...
int main() {
    ...
    rep(i, n) cin >> a[i];
    multiset<int> ms;
    rep(i, k) ms.insert(a[i]);
    vi resmx(n-k+1),resmn(n-k+1);
    int x = 0, y = 0;
    resmx[x++]=*ms.rbegin(),resmn[y++]=*ms.begin();
    debug(ms)
    ...
    rep(i, n-k+1) cout << resmn[i] << sp; cout << nl;
    rep(i, n-k+1) cout << resmx[i] << sp; cout << nl;
